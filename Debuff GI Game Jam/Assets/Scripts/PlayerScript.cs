...
public class PlayerScript : MonoBehaviour {
	...
	protected Level[] levels = new Level[]{
		...
		new Vector2[]{new Vector2(-1.4F, 4.3F), new Vector2(4F, 0F)}), // new Vector2(-3.0F, 0.5F)}),
...
*/
		...
		new Level("BossFight", new Vector2(-2, -7),
			new string[]{"Prefabs/Boss"},
			new Vector2[]{new Vector2(-2, 18)})
	...
	void Update () {
		...
		if (Input.GetButtonDown ("Dash")) {
			Debug.Log ("Dashing!");
			GetComponent<Rigidbody2D> ().velocity = (mouse.normalized * 10);
			Stun (0.25F);
		}
